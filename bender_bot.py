# bender_bot.py — с изредка напоминанием в нерабочее время
import os
import sys
import json
import random
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== НОВАЯ ЛОГИКА НЕРАБОЧЕГО ВРЕМЕНИ =====
    if not is_working_hours():
        # Если позвали или есть триггер — отвечаем всегда (даже в нерабочее время)
        if is_mentioned or is_reply_to_bot or has_trigger:
            print(f"📨 Нерабочее время, но позвали/триггер: {text}", file=sys.stderr)
            # Пропускаем через обычную логику (но без OpenAI)
        else:
            # С вероятностью 15% отвечаем напоминанием об отдыхе
            if random.random() < OFF_HOURS_CHANCE:
                await update.message.reply_text("⏰ Я отдыхаю. Приходи с 9 до 23 по будням.")
                update_response_time()
            else:
                print(f"📨 Нерабочее время, молчу: {text}", file=sys.stderr)
            return
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Проверка рабочего времени с вероятностью ответа
    if not is_working_hours():
        # Если картинка в группе — проверяем, позвали ли бота
        if update.message.chat.type in ['group', 'supergroup']:
            if not context.bot.username in update.message.caption:
                print("📨 Нерабочее время, картинка без упоминания — игнорирую", file=sys.stderr)
                return
...
if __name__ == "__main__":
    ...
    print(f"💤 Шанс напомнить о себе в нерабочее время: {int(OFF_HOURS_CHANCE * 100)}%")
